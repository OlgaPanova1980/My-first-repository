    ...
    while True:
        ...
        if mode == 1:
            ...
            if num == None:
                ...
                сurr_cost = data[week][num]["cost"]
                cost = input(f"  Введите стоимость валюты ({сurr_cost}):  ")
                if cost:
                    data[week][num]["cost"] = float(cost)

                сurr_amount = data[week][num]["amount"]
                amount = input(f"  Введите количество валюты ({сurr_amount}):  ")
                if amount:
